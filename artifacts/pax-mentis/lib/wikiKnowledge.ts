...
export const WIKI_CHUNKS: WikiChunk[] = [
  ...
  },
  {
    ...
    content: "Sheeran (2002) meta-analizi: Niyet ile davranış arasındaki korelasyon r=0.53 (varyansın ~%28'i). Bu, güçlü niyete sahip insanların önemli bir bölümünün harekete geçemediği anlamına gelir. 'İyi niyetlerin yolu cehenneme çıkar.' Niyeti eyleme dönüştürmek için özel bir psikolojik araç gerekir — uygulama niyeti.",
    contentEn: "Sheeran (2002) meta-analysis: correlation between intention and behavior is r=0.53 (~28% of variance). This means a substantial proportion of people with strong intentions fail to act. Good intentions alone aren't enough — a specific psychological tool is needed to bridge the gap.",
  ...
  },
  {
    ...
    content: "Normalizasyon: Mücadelenin yaygın olduğunu bildirmek yükü hafifletir. Üniversite öğrencilerinin %80-95'i ara sıra erteleme yaşar (Steel, 2007); ancak kronik, yaşam kalitesini düşüren erteleme %15-20 oranında görülür. Yalnız değilsin — ama bu normal olmak, hareketsiz kalmak anlamına da gelmiyor.",
    contentEn: "Normalization: Acknowledging that struggle is common reduces the burden. 80-95% of college students experience procrastination occasionally (Steel, 2007); however, chronic procrastination that lowers quality of life affects 15-20%. You're not alone — but 'normal' doesn't mean staying stuck.",
  ...
  },
  {
    ...
    content: "Karar Yorgunluğu: Gün boyunca arka arkaya alınan kararların sonraki seçim kalitesini düşürdüğü klinik gözlemdir. Baumeister'in 'ego tükenmesi' mekanizması — her kararın sabit bir öz-kontrol kapasitesinden beslenmesi — bazı büyük ölçekli çalışmalarda (Hagger et al., 2016) replicanmamıştır; mekanizma tartışmalıdır. Bununla birlikte pratik örüntü gerçektir: gün sonunda büyük kararlar almak ve belirsiz görevleri başlatmak güçleşir.",
    contentEn: "Decision Fatigue: A clinical observation that back-to-back decisions through the day reduces the quality of subsequent choices. Baumeister's 'ego depletion' mechanism — each decision drawing from a fixed self-control capacity — failed to replicate in some large-scale studies (Hagger et al., 2016); the mechanism remains contested. Nevertheless, the practical pattern is real: making major decisions and starting ambiguous tasks becomes harder late in the day.",
    ...
    source: "Baumeister et al. (1998). Ego depletion. JPSP. Hagger et al. (2016). A multilab pre-registered replication of the ego depletion effect. PSPB.",
  },
  ...
  },
  {
    ...
    content: "Rogers (1959) — gerçek benlik / ideal benlik gerilimini teorize etti. Higgins (1987) bunu genişleterek Benlik Uyuşmazlığı Teorisi'ni kurdu: gerçek benlik ('şu an kim olduğum') ile ideal benlik ('olmak istediğim') arasındaki mesafe depresyon ve üzüntü kaynağı; 'olmalı' benlikleri ('olmam gereken') ise kaygı ve ajitasyon üretir. Erteleme bu iki uyuşmazlıktan beslenebilir.",
    contentEn: "Rogers (1959) theorized the real-self / ideal-self tension. Higgins (1987) extended this into Self-Discrepancy Theory: the distance between real self ('who I am now') and ideal self ('who I want to be') is a source of depression and sadness; 'ought' selves ('who I should be') generate anxiety and agitation. Procrastination can feed from both discrepancies.",
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    keywords: ["beni kimse dinlemiyor", "söylesem ne olacak", "bana emrediliyor", "başkası ne düşünüyor ki", "kendi kararım olsa", "kimse anlamaz", "seni duymak istiyorum"],
    keywordsEn: ["nobody listens to me", "what if I say it", "being ordered around", "what does anyone else know", "if it were my decision", "no one understands"],
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // BÖLÜM 33: Erteleyici Tipolojileri — Tanısal Çerçeve (Sapadin / Fiore / Steel)
  // ─────────────────────────────────────────────────────────────────────────────
  {
    id: "tipoloji_01",
    theory: "Procrastination Typology",
    topic: "Erteleyici Tipi Tanısı",
    topicEn: "Procrastinator Type Diagnosis",
    keywords: ["neden hep yapıyorum bunu", "neden erteliyorum", "bu bende bir sorun mu", "hep böyle oldu", "karakterim mi bu", "erteleme tipim ne"],
    keywordsEn: ["why do I always do this", "why do I procrastinate", "is this a problem with me", "always been like this", "is this my character", "what type of procrastinator am I"],
    content: "Sapadin & Fiore (1996) ile Steel (2011) 6 erteleyici tipi tanımlar. 1. Mükemmeliyetçi: 'Mükemmel yapamıyorsam başlamayacağım' — korku kaynağı başarısızlık değil, yetersiz iş çıkarmak. 2. Hayalperest: Büyük vizyon var, somut adım yok — ayrıntı ve sıkıcı uygulama korkusu. 3. Endişeli: Belirsizlik felç eder — 'ya yanlış karar verirsem' döngüsü. 4. Kriz Yaratan: 'Baskı altında daha iyi çalışırım' — son dakika adrenalinini ödül olarak kullanır. 5. Meydan Okuyan: Kurallara veya otoriteye karşı pasif direniş — görev dışarıdan dayatılmış hissi. 6. Aşırı Yüklenen: Gerçek kapasite aşımı — 'hayır' diyememe, öncelik kuramama. Her tip farklı bir müdahale gerektirir.",
    contentEn: "Sapadin & Fiore (1996) and Steel (2011) identify 6 procrastinator types. 1. Perfectionist: 'If I can't do it perfectly, I won't start' — fear source is inadequate output, not failure. 2. Dreamer: Big vision, no concrete steps — fear of detail and boring implementation. 3. Worrier: Ambiguity paralyzes — 'what if I make the wrong decision' loop. 4. Crisis-Maker: 'I work better under pressure' — uses last-minute adrenaline as reward. 5. Defier: Passive resistance to rules or authority — task feels externally imposed. 6. Over-committed: Genuine capacity overload — can't say no, can't prioritize. Each type requires a different intervention.",
    socraaticPrompt: "Bu durumu düşününce içinde hangisi var: 'mükemmel yapamam', 'nereden başlayacağımı bilmiyorum', 'ya yanlış karar verirsem', 'zaten son dakikada yaparım', 'benden isteniyor o yüzden istemiyorum', yoksa 'her şeyi yapamıyorum'?",
    socraaticPromptEn: "Thinking about this situation, which one is inside you: 'I can't do it perfectly', 'I don't know where to start', 'what if I make the wrong decision', 'I'll do it last minute anyway', 'it's demanded of me so I don't want to', or 'I can't do everything'?",
    intervention: "Tipini tanı, müdahaleyi seç: Mükemmeliyetçi → %70 kuralı; Hayalperest → ilk somut adım; Endişeli → belirsizliği küçült; Kriz Yaratan → yapay son tarih koy; Meydan Okuyan → görevi sahiplen; Aşırı Yüklenen → bir şeyi listeden çıkar.",
    interventionEn: "Identify your type, choose the intervention: Perfectionist → 70% rule; Dreamer → first concrete step; Worrier → shrink the ambiguity; Crisis-Maker → set an artificial deadline; Defier → take ownership of the task; Over-committed → remove something from the list.",
    source: "Sapadin, L. & Fiore, J. (1996). It's About Time! Viking. Steel, P. (2011). The Procrastination Equation. Harper. Fiore, N. (2006). The Now Habit. TarcherPerigee.",
  },
];
